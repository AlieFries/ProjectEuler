...
while i <= 16:
    ...
    while j <= 16:
        ##right
        product = myArray[i][j]*myArray[i+1][j]*myArray[i+2][j]*myArray[i+3][j]
        if product > largest_product:
            largest_product = product
            print i, j
        ##down
        product = myArray[i][j]*myArray[i][j+1]*myArray[i][j+2]*myArray[i][j+3]
        if product > largest_product:
            largest_product = product
            print i, j
        ##diagonal down, right
        product = myArray[i][j]*myArray[i+1][j+1]*myArray[i+2][j+2]*myArray[i+3][j+3]
        if product > largest_product:
            largest_product = product
            print i,j
        if i >=3:
...
for i in range(15,20):
    j = 0
    for j in range(0,20):
        ##left(over full range of j)
        product = myArray[i][j]*myArray[i-1][j]*myArray[i-2][j]*myArray[i-3][j]
        if product > largest_product:
            largest_product = product
    j = 0
    for j in range(0,16):
        ##down(larger values of i, j to 15)
        product = myArray[i][j]*myArray[i][j+1]*myArray[i][j+2]*myArray[i][j+3]
        if product > largest_product:
            largest_product = product
    j = 0
    for j in range(0,16):
    ##diagonal down, left(larger i values)    
        product = myArray[i][j]*myArray[i-1][j+1]*myArray[i-2][j+2]*myArray[i-3][j+3]
        if product > largest_product:
            largest_product = product
for i in range(0, 16):
